...
pytest -s -m "$*" --junit-xml=${ARTIFACT_DIR}/report.xml --log-level info --disable-warnings --no-header -vv --gherkin-terminal-reporter --kube-config=${HOME}/.kube/config --alluredir=${ARTIFACT_DIR}/allure-results
