...
KUBECTL_RUN := docker run --rm -it -v ~/.kube:/kube -e KUBECONFIG=/kube/config --network=host -v`pwd`:/host -w /host --entrypoint /bin/sh bitnami/kubectl:latest -c
HELM_RUN := docker run --rm -it -v ~/.kube:/root/.kube -e KUBECONFIG=/root/.kube/config --network=host -v`pwd`:/host -w /host --entrypoint /bin/sh alpine/helm:latest -c
