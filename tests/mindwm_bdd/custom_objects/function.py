...
class MindwmFunction(CustomObject):
    # namespace = "default"

    def delete(self, options: client.V1DeleteOptions) -> client.V1Status:
        return self.api_client.delete_namespaced_custom_object(api_group, api_version, self.namespace, "functions", self.name)

    def status(self):
        r = self.api_client.get_namespaced_custom_object_status(group = api_group, version = api_version, namespace = self.namespace, plural = "functions", name = self.name)
        return r.get('status')
